 ...
 *
 * Adapted for Dameng (DM) SQL parsing in Chat2DB Community.
 * Derived from the Oracle PL/SQL grammar; dialect-specific rules such as
 * EXPLAIN may differ from the upstream Oracle grammar.
 *
...
options {
    tokenVocab = DMLexer;
    superClass = DMParserBase;
}

@parser::postinclude {
#include <DMParserBase.h>
}
...
function_body
    : FUNCTION identifier ('(' parameter (',' parameter)* ')')? RETURN type_spec (
        ...
        | result_cache_clause
        | streaming_clause
            // see example in section "How Table Functions Stream their Input Data" on streaming_clause in DM 9i: https://docs.oracle.com/cd/B10501_01/appdev.920/a96624/08_subs.htm#20554
    )* (
        ( (IS | AS) (DECLARE? seq_of_declare_specs? body | call_spec))
...
xmltype_storage
    : STORE AS (
        ...
        | (SECUREFILE | BASICFILE)? (CLOB | BINARY XML) (
            lob_segname ('(' (TABLESPACE tablespace_name = id_expression)? lob_parameters storage_clause?')')?
